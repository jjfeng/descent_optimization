...
def run(Xl_train, Xs_train, y_train, Xl_validate, Xs_validate, y_validate, Xs_test, initial_lambda1=1, initial_lambda2=1, initial_lambda3=1, initial_lambda4=None):
    method_label = "HCSmoothLinear2"
    is_four_penalties = initial_lambda4 is not None
    print "is_four_penalties", is_four_penalties
    ...
    init_lambdas = [initial_lambda1, initial_lambda2, initial_lambda3, initial_lambda4]
    lambda_array = init_lambdas if is_four_penalties else init_lambdas[:-1]
    curr_regularization = np.array(lambda_array)
    first_regularization = curr_regularization

    problem_wrapper = SmoothAndLinearProblemWrapper(Xl_train_ordered, Xs_ordered, train_indices, y_train_ordered, four_penalties=is_four_penalties)
    ...
    for i in range(0, NUMBER_OF_ITERATIONS):
        # shrink_factor = min(MAX_STEP_SIZE, shrink_factor * 100)
...
def run_nesterov(Xl_train, Xs_train, y_train, Xl_validate, Xs_validate, y_validate, Xs_test, initial_lambda1=1, initial_lambda2=1, initial_lambda3=1, initial_lambda4=None):
    ...
    while potential_cost > current_cost:
        ...
        if potential_cost > current_cost:
            if potential_cost > 2 * current_cost:
                method_step_size *= SHRINK_SHRINK_FACTOR * 0.001
            else:
    ...
    for i in range(2, NUMBER_OF_ITERATIONS + 1):
        ...
        curr_regularization = _get_updated_lambdas(acc_regularization, method_step_size, lambda_derivatives, use_boundary=True)
...
def _get_lambda_derivatives_mini(Xl_train, y_train, Xl_validate, y_validate, beta, thetas, train_indices, validate_indices, difference_matrix, lambdas):
    ...
    is_four_penalties = len(lambdas) == 4
    ...
    inv_matrix12 = sp.linalg.pinvh(MM if not is_four_penalties else MM + lambdas[3] * DD)
    inv_matrix12_X = inv_matrix12 * M.T * Xl_train
    matrix12_to_inv = XX + lambdas[1] * eye_matrix_features - Xl_train.T * M * inv_matrix12_X
    # zero_indices = np.less(np.abs(matrix12_to_inv), 1e-13)
    # matrix12_to_inv[zero_indices] = 0
    # matrix12_post_inv = sp.linalg.pinvh(matrix12_to_inv)
    dbeta_dlambdas[0], _, _, _ = np.linalg.lstsq(matrix12_to_inv, -1 * _get_zero_or_sign_vector(beta))
    # print "dbeta_dlambdas[0] 1", dbeta_dlambdas[0]
    # dbeta_dlambdas[0] = -1 * matrix12_post_inv * _get_zero_or_sign_vector(beta)
    # print "dbeta_dlambdas[0] 2", dbeta_dlambdas[0]
    dtheta_dlambdas[0] = -1 * inv_matrix12_X * dbeta_dlambdas[0]
    # print "dbeta_dlambdas[0]", dbeta_dlambdas[0]
    # print "dtheta_dlambdas[0]", dtheta_dlambdas[0]

    dbeta_dlambdas[1], _, _, _ = np.linalg.lstsq(matrix12_to_inv, -1 * beta)
    dtheta_dlambdas[1] = -1 * inv_matrix12_X * dbeta_dlambdas[1]
    # print "dtheta_dlambdas[1]", dtheta_dlambdas[1]
    # print "dbeta_dlambdas[1]", dbeta_dlambdas[1]

    inv_matrix34 = sp.linalg.pinvh(XX + lambdas[1] * eye_matrix_features)
    inv_matrix34_X = inv_matrix34 * Xl_train.T * M
    print "MM", MM
    matrix34_to_inv = MM - M.T * Xl_train * inv_matrix34_X
    # zero_indices = np.less(np.abs(matrix34_to_inv), 1e-10)
    # matrix34_to_inv[zero_indices] = 0
    if is_four_penalties:
        matrix34_to_inv += lambdas[3] * DD

    signDtheta_with_zeros = _get_zero_or_sign_vector(difference_matrix * thetas)
    # dtheta_dlambdas[2], _, _, _ = np.linalg.lstsq(matrix34_to_inv, -1 * difference_matrix.T * signDtheta_with_zeros)
    dtheta_dlambdas[2] = -1 * sp.linalg.pinvh(matrix34_to_inv) * difference_matrix.T * signDtheta_with_zeros
    dbeta_dlambdas[2] = -1 * inv_matrix34_X * dtheta_dlambdas[2]
    print "dtheta_dlambdas[2]", dtheta_dlambdas[2]
    print "dbeta_dlambdas[2]", dbeta_dlambdas[2]

    if is_four_penalties:
        dtheta_dlambdas[3], _, _, _ = np.linalg.lstsq(matrix34_to_inv, -1 * DD * thetas)
        dbeta_dlambdas[3] = -1 * inv_matrix34_X * dtheta_dlambdas[3]
    ...
    return np.array(df_dlambdas), dtheta_dlambdas, dbeta_dlambdas

# def __main__():
#     Xs_train_assert = np.matrix([1,2,9])
#     Xs_validate_assert = np.matrix([8,4,6])
#     Xs_test_assert = np.matrix([7,5,3])
#     Xs_ordered_assert, order_indices_assert, train_indices_assert, validate_indices_assert = _get_order_indices(Xs_train_assert, Xs_validate_assert, Xs_test_assert)
#     assert(np.array_equal(Xs_ordered_assert, np.matrix([1,2,3, 4, 5, 6, 7, 8, 9])))
#     assert(np.array_equal(order_indices_assert, np.matrix([0, 1, 8, 4, 6, 5, 7, 3, 2])))
