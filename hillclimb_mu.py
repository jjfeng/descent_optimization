...
def run(X_train, W_train, y_train, X_validate, W_validate, y_validate):
    method_label = "MU"
    mu_guesses = np.array([0 for i in range(0, 4)])

    problem_wrapper = EffectsInteractionProblemWrapper(X_train, W_train, y_train)

    beta_guess, theta_guess = problem_wrapper.solve_mu(mu_guesses)
    ...
    best_regularization = mu_guesses
    ...
    for i in range(0, NUMBER_OF_ITERATIONS):
        ...
        df_dlambdas = _get_lambda_derivatives(X_train, W_train, y_train, X_validate, W_validate, y_validate, beta_guess, theta_guess, mu_guesses)

        prev_lambdas = mu_guesses
        mu_guesses = _update_lambdas(mu_guesses, method_step_size, df_dlambdas)

        if np.array_equal(prev_lambdas, mu_guesses):
        ...
        print method_label, "current_cost:", current_cost, "best cost", best_cost, "lambdas:", mu_guesses
...
def _update_lambdas(lambdas, method_step_size, df_dlambdas):
    return np.minimum(lambdas - method_step_size * df_dlambdas, 10)  # prevent number overflow

def _get_lambda_derivatives(X_train, W_train, y_train, X_validate, W_validate, y_validate, beta_guess, theta_guess, mu_guesses):
    ...
    if beta_mini.size == 0:
        dbeta_dlambdas, dtheta_dlambdas = _get_dbeta_theta_dlambdas(X_train, W_train_mini, y_train, beta_guess, theta_mini, mu_guesses)
        ...
        dbeta_dlambdas, dtheta_dlambdas = _get_dbeta_theta_dlambdas(X_train_mini, W_train, y_train, beta_mini, theta_guess, mu_guesses)
        def _get_df_dlambda(lambda_idx):
        ...
        dbeta_dlambdas, dtheta_dlambdas = _get_dbeta_theta_dlambdas(X_train_mini, W_train_mini, y_train, beta_mini, theta_mini, mu_guesses)
        def _get_df_dlambda(lambda_idx):
...
def _get_dbeta_theta_dlambdas(X_train, W_train, y_train, beta, theta, mu_guesses):
    XX = X_train.T * X_train
    WW = W_train.T * W_train
    XW = X_train.T * W_train
    WX = XW.T

    WW_lambda4_inv = np.linalg.pinv(WW + np.exp(mu_guesses[3]) * np.matrix(np.identity(WW.shape[0])))
    WW_lambda4_inv_WX = WW_lambda4_inv * WX

    X_eye_matrix = np.matrix(np.identity(beta.size))

    to_invert_matrix_beta = XX + np.exp(mu_guesses[2]) * X_eye_matrix - XW * WW_lambda4_inv_WX

    dbeta_dlambda1, _, _, _ = np.linalg.lstsq(to_invert_matrix_beta, -1 * np.exp(mu_guesses[0]) * np.sign(beta))
    dtheta_dlambda1 = -1 * WW_lambda4_inv_WX * dbeta_dlambda1

    dbeta_dlambda3, _, _, _ = np.linalg.lstsq(to_invert_matrix_beta, -1 * np.exp(mu_guesses[2]) * beta)
    dtheta_dlambda3 = -1 * WW_lambda4_inv_WX * dbeta_dlambda3

    XX_lambda3_inv = np.linalg.pinv(XX + np.exp(mu_guesses[2]) * np.matrix(np.identity(XX.shape[0])))
    XX_lambda3_inv_XW = XX_lambda3_inv * XW

    W_eye_matrix = np.matrix(np.identity(theta.size))

    to_invert_matrix_theta = WW + np.exp(mu_guesses[3]) * W_eye_matrix - WX * XX_lambda3_inv_XW

    dtheta_dlambda2, _, _, _ = np.linalg.lstsq(to_invert_matrix_theta, -1  * np.exp(mu_guesses[1]) * np.sign(theta))
    dbeta_dlambda2 = -1 * XX_lambda3_inv_XW * dtheta_dlambda2

    dtheta_dlambda4, _, _, _ = np.linalg.lstsq(to_invert_matrix_theta, -1 * np.exp(mu_guesses[3]) * theta)
